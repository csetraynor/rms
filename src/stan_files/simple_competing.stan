...
model {
  // linear predictor
  vector[nevent01] eta_event01; 
  vector[nrcens01] eta_rcens01; 
  vector[nevent02] eta_event02; 
  vector[nrcens02] eta_rcens02; 
  vector[nevent12] eta_event12; 
  vector[nrcens12] eta_rcens12; 
  ...
  // define linear predictor
   ...
   /*
   // type 1
   target +=  mspline_log_surv(eta_rcens01, ibasis_rcens01, coefs01);
   target +=  mspline_log_surv(eta_rcens02, ibasis_rcens02, coefs02);
   
   // type 2
   target +=  mspline_log_haz(eta_event01,  basis_event01, coefs01);
   target +=  mspline_log_surv(eta_rcens02, ibasis_rcens02, coefs02);
   target +=  mspline_log_surv(eta_rcens12, ibasis_rcens12, coefs12);
   
   // type 3
   target +=  mspline_log_surv(eta_rcens01, ibasis_rcens01, coefs01);
   target +=  mspline_log_haz(eta_event02,  basis_event02, coefs02);
   
   // type 4
   target +=  mspline_log_haz(eta_event01,  basis_event01, coefs01);
   target +=  mspline_log_surv(eta_rcens02, ibasis_rcens02, coefs02);
   target +=  mspline_log_haz(eta_event12,  basis_event12, coefs12);
   */
   if(type01 == 1){ // weibull
  ...
  /*for(i in 1:h){
    if (type[i] == 1){
       target +=  mspline_log_cdf(eta_rcens01, ibasis1_rcens, coefs1);
       target +=  mspline_log_cdf(eta_rcens02, ibasis2_rcens, coefs2);
     }
     else if (type[i] == 2) {
       target +=  mspline_log_haz(eta_event01,  basis1_event, coefs1);
       target +=  mspline_log_cdf(eta_rcens02, ibasis2_rcens, coefs2);
       target +=  mspline_log_cdf(eta_rcens12, ibasis3_rcens, coefs3);
     }
     else if (type[i] == 3) {
       target +=  mspline_log_cdf(eta_rcens01, ibasis1_rcens, coefs1);
       target +=  mspline_log_haz(eta_event02,  basis2_event, coefs2);
     }
     else if (type[i] == 4) {
       target +=  mspline_log_haz(eta_event01,  basis1_event, coefs1);
       target +=  mspline_log_cdf(eta_rcens02, ibasis2_rcens, coefs2);
       target +=  mspline_log_haz(eta_event12,  basis3_event, coefs3);
     }
  }*/
}
...
generated quantities {
  ...
  aux01 = coefs01 * exp(log_crude_event_rate01 - dot_product(x_bar01, beta01)); // m-splines
