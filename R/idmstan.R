...
idmstan <- function(formula01,
  ...
  x_stuff01 <- make_x(formula01$tf_form, mf01)
  x01          <- x_stuff01$x
  x_bar01      <- x_stuff01$x_bar
  x_centered01 <- x_stuff01$x_centered  
  x_event01 <- keep_rows(x_centered01, status01 == 1)
  x_lcens01 <- keep_rows(x_centered01, status01 == 2)
  x_rcens01 <- keep_rows(x_centered01, status01 == 0)
  x_icens01 <- keep_rows(x_centered01, status01 == 3)
  x_delay01 <- keep_rows(x_centered01, delayed01)
  K01 <- ncol(x01)
  
  
  x_stuff02 <- make_x(formula02$tf_form, mf02)
  x02          <- x_stuff02$x
  x_bar02      <- x_stuff02$x_bar
  x_centered02 <- x_stuff02$x_centered  
  x_event02 <- keep_rows(x_centered02, status02 == 1)
  x_lcens02 <- keep_rows(x_centered02, status02 == 2)
  x_rcens02 <- keep_rows(x_centered02, status02 == 0)
  x_icens02 <- keep_rows(x_centered02, status02 == 3)
  x_delay02 <- keep_rows(x_centered02, delayed02)
  K02 <- ncol(x02)
  
  
  x_stuff12 <- make_x(formula12$tf_form, mf12)
  x12          <- x_stuff12$x
  x_bar12      <- x_stuff12$x_bar
  x_centered12 <- x_stuff12$x_centered  
  x_event12 <- keep_rows(x_centered12, status12 == 1)
  x_lcens12 <- keep_rows(x_centered12, status12 == 2)
  x_rcens12 <- keep_rows(x_centered12, status12 == 0)
  x_icens12 <- keep_rows(x_centered12, status12 == 3)
  x_delay12 <- keep_rows(x_centered12, delayed12)
  K12 <- ncol(x12)
